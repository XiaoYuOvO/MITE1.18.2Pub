...
## 其他
* 移除了`test`命令
* 现在Fabric客户端不会同步模组文件夹下的仅服务端模组
## Bug修复
* 修复了战利品箱会在玩家打开前生成战利品的Bug
* 修复了栅栏门和测重压力板在某些情况下会默认含水的Bug
* 修复了在服务器列表中无法查看服务器游戏时间的Bug
* 修复了单人游戏退出重进后,异步寻路服务未正确重启的Bug
* 修复了部分渲染mod的兼容性问题
...
# v0.7.2.2-beta 更新日志
## 物品
* 现在食用蛋糕会获得16000的蛋白质和800的胰岛素抵抗
* 现在苹果具有4800的胰岛素抗性
## 世界生成
* 现在在战利品箱中将生成带药水效果的锈铁箭而不是药箭
* 友好生物的生成上限从8个每区块改为16个
## 游戏机制
* 现在蓝月时怪物不会在地表生成
## 其他
* 服务端现在可以通过修改`server.properties`中的`sync-mods`字段来控制是否开启模组校验
## Bug修复
* 修复了两格花不是可施肥方块的问题
* 修复了退出世界后异步寻路线程池未退出导致的游戏进程无法及时退出
* 修复了流浪者在生成后手上未持有弓的Bug
* 修复了盾牌在被斧子击穿后将盾牌冷却覆盖为更低的值的Bug
* 修复了蓝月不刷新可生成动物数量的Bug
* 修复了非怪物生物也会受地表怪物生成限制的影响的Bug
* 修复了金属桶,工具材料,可配置矿物生成和烧制配方相关代码的模组兼容问题

---

# v0.7.2.1-beta 更新日志
## 怪物
* 现在怪物的复仇寻敌AI(被攻击后锁定攻击者)将不再永久锁定一位玩家直至无法追踪, 而是仅强制追踪攻击者30秒, 若此时间内又被攻击, 则缩短6秒强制追踪时间
* 怪物的非锁定寻敌AI现在会自动将目标转换为距离最近的目标
## Bug修复
* 修复了在矿物生成设置界面中文本编辑组件多个选中的Bug
* 修复了盔甲在拥有高品质且耐久损耗过多时护甲加成不正确的问题
* 修复了在服务端上游玩时无法制作高品质物品的Bug
* 修复了连续合成时, 第一个物品合成完后进度条消失的问题
* 修复了可以将矿物生成设置的尺寸和最大数量设置为限制之外的值的Bug
* 修复了在较旧版本的启动器中无法启动服务端的Bug

---

# v0.7.2-beta 更新日志
## 方块
* 现在粘液块可以在被活塞推动时正常粘附周围方块进行移动
* 现在通过桶刚放下的流动熔岩流体遇水时会变为黑曜石, 以补偿1.6.4版本时的刷黑曜石特性
## 物品
* 为工具,武器,防具添加了物品品质, 可在合成时通过右键合成产物切换合成目标品质
  + 品质每提升一级, 合成需要的经验翻倍, 合成时间倍数+2, 总耐久+50%
* 现在斧子类工具可以用于破坏沙岩类方块
* 现在四个西瓜片才能合成一个西瓜种子
* 添加了通过燧石小刀和黑曜石小刀制作工作台的配方
* 现在烹饪马铃薯和海带不再提供经验值
* 现在可以通过在副手持火把, 主手持弩的方式将火把装载在弩上将火把发射出去, 以远程放置火把
* 现在材料为纯下界合金, 钨, 艾德曼的物品具有熔岩抗性
* 三叉戟现在可以提高1.75格触及距离
* 三叉戟, 弩, 海龟壳的附魔经验上限均变为5300点
* 西瓜片的最大堆叠数量改为16个
## 界面
* 现在在工作台界面制作物品时按住Shift可以查看合成剩余的具体时间
* 现在可以在创建世界页面的更多选项页面中设置此存档中各个矿物的生成参数
  + 放置设置配置的是矿物在世界生成时进行放置位置判定时的参数\
    稀有度: 决定矿物在生成的位置成功放置的概率, 每处矿物有`1/稀有度`的概率不进行生成, 为整数\
    数量: 每个区块中矿物生成的数量, 为以`-`分隔的整数范围闭区间\
    高度范围: 决定矿物在世界中生成的高度范围和分布形式, 表达式为`[高度][连接符][高度]`
    * 连接符: `~`表示三角分布, 越靠近底部, 分布越多; `=` 表示均匀分布, 分布概率在区间内均匀
    * 高度: `^n`表示距世界底部n格; `vn`表示距世界顶部n格
    * 如`^0 ~ v32`表示在世界底部0格到世界顶部下32格内的三角分布; `10 = v10`表示在10格到距世界顶部10格内的均匀分布
  + 生成设置配置的是矿物进行具体生成时的参数:\
    最大数量: 该矿物在每处生成时最多生成多少个方块, 为整数, 0代表无限制\
    尺寸: 矿物生成时的矿脉尺寸半径, 为整数, 最大为64\
    裸露消失概率: 矿物生成在空气旁时有多大的概率不生成, 为[0,1]的小数
## 生物
* 稍微提高了幽魂的可见度, 即使在无光的地方也不是完全隐形, 同时其攻击无法再击退玩家, 血量由18点降低至15点
* 现在溺尸在攻击时有一半的概率附加窒息效果, 而不是必定附加
* 添加了`doMobEnchant`游戏规则, 用于控制怪物生成时的装备是否会被附魔, 默认为开启(即带有附魔)
* 怪物的垂直方向消失所需距离提高至水平方向消失所需距离的一半(原先为水平消失距离的四分之一)
* 怪物生成时继续拥有下一件装备的概率现在受游戏难度的影响, 具体为: `75% + 5% * 难度 + 0.15% * 天数`(难度数值对于和平为0, 困难为3)
* 现在怪物的护甲永远为其基础护甲与装备护甲总和的70%-40% (根据游戏难度设置线性变化)
* 现在山羊也可以通过对其使用桶或碗来接奶, 其产奶速度和总量与牛相同
* 鱿鱼在非海洋类群系的每区块生成上限改为1, 每批生成数量改为1只, 生成权重变为1, 并且其追踪距离从32下降至24
* 怪物类生物的基础上限调高至35只, 水生生物的基础上限降低至3只
## 世界生成
...
* 现在海草的堆叠上限改为32个
* 现在可以在金属砧中使用金属粒修复盾牌
* 现在每个虫子都独立计算其物品栏内的游戏刻,并以此吃掉树叶或食物,产生粪便
## 生物
* 现在大地元素不再免疫铁傀儡的攻击
* 现在恶魂攻击玩家的垂直距离从4格增加到12格
* 相位蜘蛛的垂直传送距离降至32格
* 地底世界生成的蜘蛛骑士现在会随机古尸或远古骷髅领主(10%)作为骑手
* 各类吸血蝙蝠现在不会攻击水下的玩家
* 海龟会主动食用丢在地上的海草
## 世界生成
* 现在地底世界不再会生成僵尸村民
## 游戏机制
* 现在玩家在重度胰岛素反应时食用含胰岛素反应的食物将会受到中毒效果
* 现在蓝月会给予玩家1点幸运值
## Bug修复
* 修复了僵尸在挖掘砂砾后崩溃的Bug
* 修复了远古残骸不受时运附魔影响的Bug
* 修复了部分动物掉落的肉的数量不受屠宰附魔影响而受抢夺附魔影响的Bug
* 修复了骷髅掉落的是燧石箭而不是锈铁箭的Bug
* 修复了近战骷髅也会掉落箭矢的Bug
* 修复了猪灵不喜欢新加入的金质物品的Bug
* 修复了铁盔甲合成等级不正确的Bug
* 修复了烟熏炉烤制肉类没有经验的Bug
* 修复了进度"热腾腾的"只检测铁熔岩桶的Bug
* 修复了玩家被无来源爆炸炸死时崩溃的Bug
* 修复了雪球无法对烈焰人造成伤害的Bug
* 修复了骷髅领主有时无法生成骷髅的Bug
* 修复了恶魂由于距离玩家垂直距离过高而消失的Bug
* 修复了血月白天会有非苦力怕和蜘蛛怪物的生成的Bug
* 修复了相位蜘蛛由于瞬移距离玩家过远而消失的Bug
* 修复了猫不会在睡醒后给予玩家礼物的Bug
* 修复了玩家死亡后诅咒消失的Bug
* 修复了玩家死亡后胰岛素抵抗值重置的Bug
* 修复了箭矢在击中无法造成伤害的实体后弹落在地上并在目标实体上累计箭矢数的Bug
* 修复了无来源伤害触发盾牌时崩溃的Bug
* 修复了不祥之兆在触发后不会被移除的Bug
* 修复了使用桶对含水方块捞起源头时使方块消失的Bug
* 修复了生物会无视生成上限生成的Bug
